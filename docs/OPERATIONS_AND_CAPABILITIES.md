...
- Each host checks the latest published GitHub Release for a newer version with a complete, checksummed platform asset. A discreet logo badge appears only for that release; Settings shows the current release status and has **Check now**. Rootless Fedora installations download and verify the Linux archive, then update the user service after sessions are idle; startup health is checked, failure rolls back, and open desktop windows from the old version reopen with the new code. Windows and RPM installations link to the release for installer-based updating. The existing **Agent runtime updates** switch controls Codex, Claude and OpenCode, not CIEL app updates.

...
- Rootless Fedora installations can now check a host-local releases folder set in Settings. A discreet logo badge appears for a newer Linux tarball and starts a user-service update after sessions are idle. The updater stages the new app, checks its health and rolls back if startup fails. It does not fetch builds itself; each host needs the release archive in its configured folder. The existing **Agent runtime updates** switch controls Codex, Claude and OpenCode, not CIEL app updates.
